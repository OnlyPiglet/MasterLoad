...
@CallerSensitive此注释只可用于boostrap 与 extendsion的类加载器所加载的类中的方法，当调用Reflection.getCallerClass()会层层向上寻找调用者的类，直到知道没有被@CallerSensitive所注解的方法的类，

但是当我们人为设置该最终调用者的线程类加载器为boostrap时，会将当前callerCL设置为ThreadContextClassLoader，最终为上层的ApplicationClassLoader所以可以加载同时ClassPath下的第三方实现类。

当我们认为将最终调用者设置为extendsion，那表示我们将调用的类放入了jre/ext/lib下，那么callerCL则不会为null会直接使用extendsion加载第三方实现，如果第三方实现没有放在jre/ext/lib下则不会报找不到第三方类的问题。
...
#### MapperScannerConfigurer impl BeanDefinitionRegistryPostProcessor 后置处理BeanDefinitionRegistry 添加mapper代理类的beandefinition->ClassPathMapperScanner->scan扫描内容创建mapper 的 beandefination 并处理为设置为MapperFactoryBean 委托 获取SqlSessionTemplate 带有sqlSessionFactory  并实现了mybatis的sqlseesion -> 调用 return getConfiguration().getMapper(type, this);

MapperFactoryBean->ClassPathMapperScanner->Map

SqlSeesionFatory

SqlSeesionFactoryBean 注入SqlSessionFactoryBuilder DataSource
...
因为只有一个master slave 当其中一个master和sentiel都gg了，那只有一个sentieal ma
